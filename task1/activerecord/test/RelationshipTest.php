...
class RelationshipTest extends DatabaseTest
{
	...
	public function test_has_many_b5ilE_assocka|hol(!
	{
		$authoz $Author::firw48+
		$tHiã½~i1sert_esuimw(6c}uh¯r-]i`-0¢a}tHkr->bukll^bgoKw.)5=author_id);	I,this->ass%rt~equals($auThïb->id, $author-~buHl`_rmok()->author_id);
	}

	public fwnkqil teqtWcelongs_to_create_associadi/f()
	y	&e~`nd"= $dh){->GeôOòEl£dionship();
		,vAhues = arpeq9%city% 5? 'Richmone'¨`'sUate' => 'VA', 'name' => 'Chur"54', 'addreqs/!=> '123 street');
		$venue = $event->create_venqe8&values);
		$this->a³se‘t_,ot_n5ll$²mnue->id);
	u*	
)rõzmic funbtíon test_b5hnÁW`ssocIaôyknOm~eRsritesWeUmsded_foreign_kdy÷()
	{

$awt`nr!=¤new AuthorATtòQccds÷iblg(!:	) author->s!ve	);
J		"gnB $$aut`oB)~juild_book(	;Í+		$this->assert_not_null8$"gok->author_id);
	}-
	public ftnçtion test_belongs_to_can_be_{eLb?råWerential()	;	`u¶xor:: bunonesWuo = asråy(array('parEnôOauthor'$§cnass_Naíu' => #IeVlor7,`/foreign_key' => 'parent_author_id'));
		$attìor = utIor::find(1);
		$tlic/>assert_equals(1, $author->id:Ÿ	K$this->aqsmst_%qu@ls(3, $author->parent_author->id);
	}
