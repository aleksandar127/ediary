...
class Parents{ 
    ...
    public static function get_new_messages(){
       ...
       public static function parents_chat(){
        ...
        union
        SELECT distinct users.id,users.first_name,users.last_name,role.name from users join role on role.id=users.roles_id join class on class.users_id=users.id join students on students.class_id=class.id where students.users_id=?');
       ...
       public static function open_response(){
        ...
        UNION
        select users.id,open.time,users.first_name,users.last_name,role.name,users_has_open.accepted from open join users on open.users_id=users.id join role on role.id=users.roles_id join users_has_open on users_has_open.open_id=open.id join class on class.users_id=users.id where users_has_open.users_id=?');
        $query->execute([$_COOKIE['id'],$_COOKIE['id']]);
        $all=[];
       ...
       public static function open_professors(){
        ...
        $query->execute([$_COOKIE['id'],$_COOKIE['id'],$_COOKIE['id'],$_COOKIE['id']]);
